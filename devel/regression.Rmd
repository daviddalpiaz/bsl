...
**Chapter Status:** Currently outlining topics. Code is not using "best practices." Formatting is not considering, especially any PDF rendering. Many FIXME and TODO labels to be updated.

***

- TODO: consider moving some of this to an introduction section?

- FIXME: This chapter will set the stage for the entire course.
    - This chapter will introduce one of three main tasks: regression
    - what are the three tasks? regression, classification, and ?? density estimation? clustering?
        - density estimation really is the basis of everything isn't it?
        - all of supervised is sort of just regression, but we will distinguish between regression (numeric outcome) and classification (categorical outcome)

- TODO: Keywords
    - Data Generating Process (Move to previous chapter?)
...
- TODO: remove this from the TOC
- TODO: aggregate at book level
...
- TODO; data generating processes to be used throughout this chapter:
    - 1-dim linear relationship
        - with and without Heteroscedasticity, maybe only with
            - note how this affects prediction
    - 1-dim sin
    - something with "jumps"
    - something in 2D that can be visualized
...
$$
- TODO: RSS vs $R^2$ vs RMSE, why MSE?
...
par(mfrow = c(1, 3))
plot(y ~ x, data = sim_trn_data, col = "darkgrey", pch = 20, 
     main = "Tree Model, cp = 0.5", cex = 1.5)
grid()
lines(x_grid$x, tree_fit_l_pred, col = "dodgerblue",  lwd = 2)

plot(y ~ x, data = sim_trn_data, col = "darkgrey", pch = 20,
     main = "Tree Model, cp = 0.015", cex = 1.5)
grid()
lines(x_grid$x, tree_fit_m_pred, col = "dodgerblue",  lwd = 2)

plot(y ~ x, data = sim_trn_data, col = "darkgrey", pch = 20, 
     main = "Tree Model, cp = 0.0", cex = 1.5)
grid()
lines(x_grid$x, tree_fit_h_pred, col = "dodgerblue",  lwd = 2)
```
...
- TODO: check this against old BVT chapter
...
$$
\text{rmse}\left(\hat{f}, \mathcal{D}\right) = \sqrt{\frac{1}{n}\displaystyle\sum_{i = 1}^{n}\left(y_i - \hat{f}({\boldsymbol{x}}_i)\right)^2}
$$

$$
\text{RMSE}_{\texttt{trn}} = \text{rmse}\left(\hat{f}, \mathcal{D}_{\texttt{trn}}\right) = \sqrt{\frac{1}{n_{\texttt{trn}}}\displaystyle\sum_{i \in {\texttt{trn}}}^{}\left(y_i - \hat{f}({\boldsymbol{x}}_i)\right)^2}
$$
...
- TODO: hold `cp` constant, alter `minsplit`
- TODO: in many ways, classification is just regression...
- TODO: y_hat_i = f_hat(x_i)


- linear
- quadratic
- sin
- two "groups" that a tree gets right

vs

- SLR, quadtratic, sin?, other var?
- knn: low, medium, high
- tree: stump, middle, huge
...
```{r}
## define data generating processs
gen_sim_data = function(f, sample_size = 100, x_min = -5, x_max = 5, sigma = 1) {
  x = runif(n = sample_size, min = x_min, max = x_max)
  y = rnorm(n = sample_size, mean = f(x), sd = sigma)
  data.frame(x, y)
}
```
...
## simualte train and test data
set.seed(42)
sim_trn_data = gen_sim_data(f = f, sample_size = 100, sigma = 0.15)
sim_tst_data = gen_sim_data(f = f, sample_size = 100, sigma = 0.15)

## create grid for plotting
x_grid = data.frame(x = seq(-5, 5, 0.001))
```
...
plot(y ~ x, data = sim_trn_data, col = "darkgrey", pch = 20, 
     main = "KNN, k = 1", cex = 1.5)
grid()
curve(sin(x), add = TRUE, lwd = 2)
curve(predict(tree_fit_m, newdata = data.frame(x = x)), add = TRUE, col = "dodgerblue", lwd = 2)
```
